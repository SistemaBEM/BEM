...
<html lang="en">
  ...
  <body>
    ...
    <div class="container">       
        ...
        <div class="jumbotron">
        <form  id="contactForm" data-toggle="validator" role="form" method="post" action="/BEM-projeto/ServletValidarComentario">
        <table class="table">
                <thead>
                    <tr>
                        <th><center>Nome</center></th>
                        <th><center>E-Mail</center></th>
                        <th><center>Forma de Atendimento</center></th>
                        <th><center>Validação</center></th>
                    </tr>
                 </thead>
                <tbody>
                <c:forEach var="ap" items="${listaUsuarios}" varStatus="ordem">
                    <tr>
                        <td><center>${ap.user.nome_completo}<input type="hidden" name="nome" value="${ap.user.nome_completo}"/></center></td>
                        <td><center>${ap.user.email}<input type="hidden" name="email" value="${ap.user.email}"/></center></td>
                        <td>
                            <center> -
                                <c:if test="${ap.ta.privado == true}"> Privado -  </c:if>
                                <c:if test="${ap.ta.amil == true}"> Amil -  </c:if>
                                <c:if test="${ap.ta.unimedNatal == true}"> Unimed Natal -  </c:if>
                                <c:if test="${ap.ta.hapvida == true}"> Hapvida -  </c:if>
                            </center>
                        </td>
                        <td>
                            <center>
                                <select name="select_validacao" class="form-control" id="exampleFormControlSelect1">
                                    <option value="nao_validado"
                                        <c:if test="${ap.ap.statusValidacaoApply == false}"> selected="true"  </c:if>
                                    >Não validado</option>
                                    <option value="validado"
                                        <c:if test="${ap.ap.statusValidacaoApply == true}"> selected="true" </c:if> 
                                    >Validado</option>
                                </select>
                            </center>
                        </td>
